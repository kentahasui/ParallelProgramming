 ...
 * CS337
 * 3/31/2015
 * Assignment 3.4: Philosophers4.upc
 * 
 * Compile: upcc -T 5 philosophers4.upc
 * Run:     upcrun philosophers4
 *
...
// Lock to flag whether there is space at the table
upc_lock_t * shared tableSpace;
...
void life_cycle(void)
{
  ...
  { 
    ...
    // Tries to sit at the table. 
    // The philosopher is blocked if there is only one seat open. 
    upc_lock(tableSpace);
    upc_lock(waiter);
    ...
    // If there is more space available, let other philosophers sit. 
    if(philCount < MYTHREAD - 1){
	upc_unlock(tableSpace);
    }
    else{
	hasLock = 1;
    }
